    ...
    if FLAGS.mode == 'chauvenet':
        ...
        for col in outlier_columns:
            ...
            for C in CRange:
                ...
                numOutliers.append(sum(dataset[f"{col}_outlier"]))
        ...
        for col in outlier_columns:
            for N in [2, 3, 4, 5]:
                ...
                dataset = OutlierDistr.mixture_model(dataset, col, N)
        ...
        for col in outlier_columns:
            fminRange = [0.5, 0.8, 0.9, 0.95, 0.99, 0.999]
            #fminRange = [0.99]
            for fmin in fminRange:
                try:
                    dataset = OutlierDist.simple_distance_based(
                        dataset, [col], 'euclidean', FLAGS.dmin, fmin)
                    print(dataset)
                    numOutliers.append(sum(dataset['simple_dist_outlier'].fillna(False)))
            ...
            print(fminRange)
            print(numOutliers)
            DataViz.plot_xy([fminRange], [numOutliers], xlabel="fmin", ylabel="number of outliers")
